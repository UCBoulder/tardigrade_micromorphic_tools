...
namespace tardigradeMicromorphicTools {
    ...
    void computePsi(const variableVector &deformationGradient, const variableVector &microDeformation,
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(deformationGradient.size() == sot_dim,
                                     "The deformation gradient doesn't have the correct size");

        TARDIGRADE_ERROR_TOOLS_CHECK(microDeformation.size() == sot_dim,
                                     "The micro-deformation doesn't have the correct size");

        Psi = variableVector(sot_dim, 0);

        Eigen::Map<const Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> > map1(deformationGradient.data(), dim,
                                                                                       dim);
        Eigen::Map<const Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> > map2(microDeformation.data(), dim,
                                                                                       dim);
    ...
    void computeGamma(const variableVector &deformationGradient, const variableVector &gradChi, variableVector &Gamma) {
        /*!
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(deformationGradient.size() == sot_dim,
                                     "The deformation gradient isn't the right size");
        ...
        for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int I = 0; I < dim; I++) {
                for (unsigned int J = 0; J < dim; J++) {
                    for (unsigned int K = 0; K < dim; K++) {
                        Gamma[dim * dim * I + dim * J + K] +=
                            deformationGradient[dim * i + I] * gradChi[dim * dim * i + dim * J + K];
                    }
                }
            }
        }
            ...
            for (unsigned int J = 0; J < dim; J++) {
                for (unsigned int K = 0; K < dim; K++) {
                    for (unsigned int l = 0; l < dim; l++) {
                        dGammadF[dim * dim * sot_dim * I + dim * sot_dim * J + sot_dim * K + dim * l + I] =
                            gradChi[dim * dim * l + dim * J + K];
                        dGammadGradChi[dim * dim * tot_dim * I + dim * tot_dim * J + tot_dim * K + dim * dim * l +
                                       dim * J + K] = deformationGradient[dim * l + I];
                    }
                }
            }
        }
    ...
    void computeMicroStrain(const variableVector &Psi, variableVector &microStrain, variableVector &dMicroStraindPsi) {
        /*!
         * Compute the microstrain defined as:
         * \f$ \Epsilon_{IJ} = \Psi_{IJ} - eye_{IJ} \f$
         *
        ...
        for (unsigned int i = 0; i < sot_dim; i++) {
            dMicroStraindPsi[sot_dim * i + i] = 1;
        };

        return;
    }

    void computeMicroStrain(const variableVector &Psi, variableVector &microStrain, variableMatrix &dMicroStraindPsi) {
        /*!
         * Compute the microstrain defined as:
         * \f$ \Epsilon_{IJ} = \Psi_{IJ} - eye_{IJ} \f$
         *
    ...
    void pushForwardPK2Stress(const variableVector &PK2Stress, const variableVector &deformationGradient,
        ...
        variableType detF;
        TARDIGRADE_ERROR_TOOLS_CATCH(pushForwardReferenceMicroStress(PK2Stress, deformationGradient, detF,
                                                                     cauchyStress));
    ...
    void pushForwardPK2Stress(const variableVector &PK2Stress, const variableVector &deformationGradient,
                              variableVector &cauchyStress, variableVector &dCauchyStressdPK2Stress,
    ...
    void pushForwardPK2Stress(const variableVector &PK2Stress, const variableVector &deformationGradient,
                              variableVector &cauchyStress, variableMatrix &dCauchyStressdPK2Stress,
    ...
    void pullBackCauchyStress(const variableVector &cauchyStress, const variableVector &deformationGradient,
    ...
    void pullBackCauchyStress(const variableVector &cauchyStress, const variableVector &deformationGradient,
         ...
         *
         * \f$ \frac{ \partial S_{IJ} }{ \partial F_{kK} } = F_{Kk}^{-1} S_{IJ} - F_{Ik}^{-1} S_{KJ} - S_{IK}
         * F_{Jk}^{-1} \f$
         *
         * \param &cauchyStress: The Cauchy stress in the current configuration of
         *     the provided deformation gradient.
    ...
    void pushForwardReferenceMicroStress(const variableVector &referenceMicroStress,
                                         const variableVector &deformationGradient, variableVector &microStress) {
        /*!
        ...
        variableType detF;
        return pushForwardReferenceMicroStress(referenceMicroStress, deformationGradient, detF, microStress);
    }

    void pushForwardReferenceMicroStress(const variableVector &referenceMicroStress,
                                         const variableVector &deformationGradient, variableType &detF,
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(referenceMicroStress.size() == sot_dim,
                                     "The reference micro-stress has an incorrect size");

        TARDIGRADE_ERROR_TOOLS_CHECK(deformationGradient.size() == sot_dim,
                                     "The deformation gradient has an incorrect size");

        microStress = variableVector(sot_dim, 0);

        Eigen::Map<const Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> > map1(deformationGradient.data(), dim,
                                                                                       dim);
        Eigen::Map<const Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> > map2(referenceMicroStress.data(), dim,
                                                                                       dim);
    ...
    void pushForwardReferenceMicroStress(const variableVector &referenceMicroStress,
        ...
        variableType detF;
        TARDIGRADE_ERROR_TOOLS_CATCH(pushForwardReferenceMicroStress(referenceMicroStress, deformationGradient, detF,
                                                                     microStress));

        // Assemble the inverse deformation gradient
        variableVector inverseDeformationGradient = deformationGradient;
        Eigen::Map<Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> > map(inverseDeformationGradient.data(), dim,
                                                                                dim);
        ...
        for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < dim; j++) {
                for (unsigned int k = 0; k < dim; k++) {
                    temp_sot1[dim * i + j] +=
                        referenceMicroStress[dim * j + k] * deformationGradient[dim * i + k] / detF;
                    temp_sot2[dim * i + j] +=
                        referenceMicroStress[dim * k + j] * deformationGradient[dim * i + k] / detF;
                }
            }
        }
        ...
        for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < dim; j++) {
                for (unsigned int k = 0; k < dim; k++) {
                    dMicroStressdDeformationGradient[dim * sot_dim * i + sot_dim * j + dim * i + k] +=
                        temp_sot1[dim * j + k];
                    dMicroStressdDeformationGradient[dim * sot_dim * i + sot_dim * j + dim * j + k] +=
                        temp_sot2[dim * i + k];

                    for (unsigned int K = 0; K < dim; K++) {
                        dMicroStressdReferenceMicroStress[dim * sot_dim * i + sot_dim * j + dim * k + K] =
                            deformationGradient[dim * i + k] * deformationGradient[dim * j + K] / detF;

                        dMicroStressdDeformationGradient[dim * sot_dim * i + sot_dim * j + dim * k + K] -=
                            microStress[dim * i + j] * inverseDeformationGradient[dim * K + k];
                    }
                }
            }
        }
    ...
    void pullBackMicroStress(const variableVector &microStress, const variableVector &deformationGradient,
        ...
        return pullBackMicroStress(microStress, deformationGradient, detF, inverseDeformationGradient,
                                   referenceMicroStress);
    }
    ...
    void pullBackMicroStress(const variableVector &microStress, const variableVector &deformationGradient,
        ...
        Eigen::Map<const Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> > cmap(deformationGradient.data(), dim,
                                                                                       dim);
            ...
            for (unsigned int J = 0; J < dim; J++) {
                for (unsigned int i = 0; i < dim; i++) {
                    referenceMicroStress[dim * I + J] +=
                        temp_sot[dim * I + i] * inverseDeformationGradient[dim * J + i];
                }
            }
        }
    ...
    void pullBackMicroStress(const variableVector &microStress, const variableVector &deformationGradient,
                             variableVector &referenceMicroStress, variableMatrix &dReferenceMicroStressdMicroStress,
         ...
         *
         * \f$\frac{ \partial \Sigma_{IJ} }{ \partial F_{kK} } = F_{Kk}^{-1} \Sigma_{IJ} - F_{Ik}^{-1} \Sigma_{KJ} -
         * \Sigma_{IK} F_{Jk}^{-1}\f$
         *
         * \param &microStress: The micro-stress in the current
         *     configuration.
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(pullBackMicroStress(microStress, deformationGradient, referenceMicroStress,
                                                         _dReferenceMicroStressdMicroStress,
        ...
        dReferenceMicroStressdDeformationGradient =
            tardigradeVectorTools::inflate(_dReferenceMicroStressdDeformationGradient, 9, 9);

        return;
    }

    void pullBackMicroStress(const variableVector &microStress, const variableVector &deformationGradient,
                             variableVector &referenceMicroStress, variableVector &dReferenceMicroStressdMicroStress,
        ...
        variableType   detF;
        variableVector inverseDeformationGradient;
        TARDIGRADE_ERROR_TOOLS_CATCH(pullBackMicroStress(microStress, deformationGradient, detF,
                                                         inverseDeformationGradient, referenceMicroStress));
    ...
    void pushForwardHigherOrderStress(const variableVector &referenceHigherOrderStress,
        ...
        variableType detF;
        return pushForwardHigherOrderStress(referenceHigherOrderStress, deformationGradient, microDeformation, detF,
                                            higherOrderStress);
    }

    void pushForwardHigherOrderStress(const variableVector &referenceHigherOrderStress,
                                      const variableVector &deformationGradient, const variableVector &microDeformation,
                                      variableType &detF, variableVector &higherOrderStress) {
        /*!
         * Compute the push-forward operation on the higher order stress.
         *
         * \f$m_{ijk} = \frac{1}{J} F_{iI} F_{jJ} \Chi_{kK} M_{IJK}\f$
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(referenceHigherOrderStress.size() == tot_dim,
                                     "The reference higher order stress doesn't have the correct size");
        ...
        Eigen::Map<const Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> > map(deformationGradient.data(), dim,
                                                                                      dim);
                ...
                for (unsigned int k = 0; k < dim; k++) {
                    for (unsigned int I = 0; I < dim; I++) {
                        higherOrderStress[dim * dim * i + dim * j + k] +=
                            deformationGradient[dim * i + I] * referenceHigherOrderStress[dim * dim * I + dim * j + k];
                    }
                }
            }
        }
        ...
        for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < dim; j++) {
                for (unsigned int k = 0; k < dim; k++) {
                    for (unsigned int I = 0; I < dim; I++) {
                        temp_tot[dim * dim * i + dim * j + k] +=
                            deformationGradient[dim * j + I] * higherOrderStress[dim * dim * i + dim * I + k];
                    }
                }
            }
        }
        ...
        for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < dim; j++) {
                for (unsigned int k = 0; k < dim; k++) {
                    for (unsigned int I = 0; I < dim; I++) {
                        higherOrderStress[dim * dim * i + dim * j + k] +=
                            microDeformation[dim * k + I] * temp_tot[dim * dim * i + dim * j + I];
                    }
                }
            }
        }
    ...
    void pushForwardHigherOrderStress(const variableVector &referenceHigherOrderStress,
         ...
         * \param &microDeformation: The micro-deformation tensor.
         * \param &higherOrderStress: The higher order stress in the current configuration.
         * \param &dHigherOrderStressdReferenceHigherOrderStress: The Jacobian of the higher order stress w.r.t. the
         * reference higher order stress \param &dHigherOrderStressdDeformationGradient: The Jacobian of the higher
         * order stress w.r.t. the deformation gradient \param &dHigherOrderStressdMicroDeformation: The Jacobian of the
         * higher order stress w.r.t. the micro deformation
         */
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(pushForwardHigherOrderStress(referenceHigherOrderStress, deformationGradient,
        ...
        dHigherOrderStressdReferenceHigherOrderStress =
            tardigradeVectorTools::inflate(_dHigherOrderStressdReferenceHigherOrderStress, 27, 27);
        dHigherOrderStressdDeformationGradient =
            tardigradeVectorTools::inflate(_dHigherOrderStressdDeformationGradient, 27, 9);
        dHigherOrderStressdMicroDeformation =
            tardigradeVectorTools::inflate(_dHigherOrderStressdMicroDeformation, 27, 9);
                ...
                for (unsigned int k = 0; k < dim; k++) {
                    for (unsigned int l = 0; l < dim; l++) {
                        for (unsigned int M = 0; M < dim; M++) {
                            dHigherOrderStressdDeformationGradient[dim * dim * sot_dim * i + dim * sot_dim * j +
                                                                   sot_dim * k + dim * i + l] +=
                            ...
                            dHigherOrderStressdMicroDeformation[dim * dim * sot_dim * i + dim * sot_dim * j +
                                                                sot_dim * k + dim * k + l] +=
                                temp_tot3a[dim * dim * i + dim * l + M] * deformationGradient[dim * j + M];

                            for (unsigned int N = 0; N < dim; N++) {
                                dHigherOrderStressdReferenceHigherOrderStress[dim * dim * tot_dim * i +
                                                                              dim * tot_dim * j + tot_dim * k +
                                                                              dim * dim * l + dim * M + N] =
                                    deformationGradient[dim * i + l] * deformationGradient[dim * j + M] *
                                    microDeformation[dim * k + N] / detF;
                            }
                            dHigherOrderStressdDeformationGradient[dim * dim * sot_dim * i + dim * sot_dim * j +
                                                                   sot_dim * k + dim * l + M] -=
                                higherOrderStress[dim * dim * i + dim * j + k] *
                                inverseDeformationGradient[dim * M + l];
                        }
                    }
                }
            }
    ...
    void pullBackHigherOrderStress(const variableVector &higherOrderStress, const variableVector &deformationGradient,
                                   const variableVector &microDeformation, variableVector &referenceHigherOrderStress) {
        /*!
        ...
        return pullBackHigherOrderStress(higherOrderStress, deformationGradient, microDeformation, detF,
                                         inverseDeformationGradient, inverseMicroDeformation,
                                         referenceHigherOrderStress);
    }

    void pullBackHigherOrderStress(const variableVector &higherOrderStress, const variableVector &deformationGradient,
                                   const variableVector &microDeformation, variableType &detF,
                                   variableVector &inverseDeformationGradient, variableVector &inverseMicroDeformation,
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(higherOrderStress.size() == tot_dim,
                                     "The current higher order stress doesn't have the correct size");

        TARDIGRADE_ERROR_TOOLS_CHECK(deformationGradient.size() == sot_dim,
                                     "The deformation gradient doesn't have the correct size");

        TARDIGRADE_ERROR_TOOLS_CHECK(microDeformation.size() == dim * dim,
                                     "The micro-deformation doesn't have the correct size");
        ...
        new (&map) Eigen::Map<Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> >(inverseMicroDeformation.data(),
                                                                                       dim, dim);
        map = map.inverse().eval();
                ...
                for (unsigned int J = 0; J < dim; J++) {
                    for (unsigned int K = 0; K < dim; K++) {
                        referenceHigherOrderStress[dim * dim * I + dim * J + K] +=
                            inverseDeformationGradient[dim * I + i] * higherOrderStress[dim * dim * i + dim * J + K];
                    }
                }
            }
        }
        ...
        std::fill(referenceHigherOrderStress.begin(), referenceHigherOrderStress.end(), 0.);
        ...
        for (unsigned int I = 0; I < dim; I++) {
            for (unsigned int J = 0; J < dim; J++) {
                for (unsigned int K = 0; K < dim; K++) {
                    for (unsigned int i = 0; i < dim; i++) {
                        referenceHigherOrderStress[dim * dim * I + dim * J + K] +=
                            inverseMicroDeformation[dim * K + i] * temp_tot1[dim * dim * I + dim * J + i];
                    }
                }
            }
        }
    ...
    void pullBackHigherOrderStress(const variableVector &higherOrderStress, const variableVector &deformationGradient,
         ...
         *
         * \f$ \frac{ \partial M_{IJK} }{ \partial F_{lL} } = F_{Ll}^{-1} M_{IJK} - F_{Il}^{-1} M_{LJK} - F_{Jl}^{-1}
         * M_{ILK} \f$
         *
         ...
         * \param &microDeformation: The micro-deformation tensor.
         * \param &higherOrderStress: The higher order stress in the current configuration.
         * \param &dReferenceHigherOrderStressdHigherOrderStress: The derivative of the reference higher order stress
         * w.r.t. the higher order stress \param &dReferenceHigherOrderStressdDeformationGradient: The derivative of the
         * reference higher order stress w.r.t. the deformation gradient \param
         * &dReferenceHigherOrderStressdMicroDeformation: The derivative of the reference higher order stress w.r.t. the
         * micro deformation
         */
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(pullBackHigherOrderStress(higherOrderStress, deformationGradient, microDeformation,
                                                               referenceHigherOrderStress,
                                                               _dReferenceHigherOrderStressdHigherOrderStress,
                                                               _dReferenceHigherOrderStressdDeformationGradient,
                                                               _dReferenceHigherOrderStressdMicroDeformation));

        dReferenceHigherOrderStressdHigherOrderStress =
            tardigradeVectorTools::inflate(_dReferenceHigherOrderStressdHigherOrderStress, 27, 27);
        dReferenceHigherOrderStressdDeformationGradient =
            tardigradeVectorTools::inflate(_dReferenceHigherOrderStressdDeformationGradient, 27, 9);
        dReferenceHigherOrderStressdMicroDeformation =
            tardigradeVectorTools::inflate(_dReferenceHigherOrderStressdMicroDeformation, 27, 9);
    ...
    void pullBackHigherOrderStress(const variableVector &higherOrderStress, const variableVector &deformationGradient,
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(pullBackHigherOrderStress(higherOrderStress, deformationGradient, microDeformation,
                                                               detF, inverseDeformationGradient,
        ...
        for (unsigned int I = 0; I < dim; I++) {
            for (unsigned int J = 0; J < dim; J++) {
                for (unsigned int K = 0; K < dim; K++) {
                    for (unsigned int l = 0; l < dim; l++) {
                        for (unsigned int m = 0; m < dim; m++) {
                            dReferenceHigherOrderStressdDeformationGradient[dim * dim * sot_dim * I +
                                                                            dim * sot_dim * J + sot_dim * K + dim * l +
                                                                            m] +=
                                inverseDeformationGradient[dim * m + l] *
                                    referenceHigherOrderStress[dim * dim * I + dim * J + K] -
                                inverseDeformationGradient[dim * I + l] *
                                    referenceHigherOrderStress[dim * dim * m + dim * J + K] -
                                inverseDeformationGradient[dim * J + l] *
                                    referenceHigherOrderStress[dim * dim * I + dim * m + K];

                            dReferenceHigherOrderStressdMicroDeformation[dim * dim * sot_dim * I + dim * sot_dim * J +
                                                                         sot_dim * K + dim * l + m] -=
                                inverseMicroDeformation[dim * K + l] *
                                referenceHigherOrderStress[dim * dim * I + dim * J + m];

                            for (unsigned int n = 0; n < dim; n++) {
                                dReferenceHigherOrderStressdHigherOrderStress[dim * dim * tot_dim * I +
                                                                              dim * tot_dim * J + tot_dim * K +
                                                                              dim * dim * l + dim * m + n] +=
                                    detF * inverseDeformationGradient[dim * I + l] *
                                    inverseDeformationGradient[dim * J + m] * inverseMicroDeformation[dim * K + n];
                            }
                        }
                    }
                }
    ...
    void computeDeviatoricHigherOrderStress(const variableVector &higherOrderStress,
        ...
        for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int k = 0; k < dim; k++) {
                for (unsigned int j = 0; j < dim; j++) {
                    deviatoricHigherOrderStress[dim * dim * i + dim * i + j] -=
                        higherOrderStress[dim * dim * k + dim * k + j] / 3;
                }
            }
        }
    ...
    void computeDeviatoricHigherOrderStress(const variableVector &higherOrderStress,
         ...
         * \f$ \frac{ \partial \text{dev} ( m_{ijk} ) }{ \partial m_{mno} } = \delta_{im} \delta_{jn} \delta_{ko} - ( 1
         * / 3 ) \delta_{mn} \delta_{ko} \delta_{ij} \f$
         *
         * \param &higherOrderStress: The higher order stress in the current configuration.
         * \param &deviatoricHigherOrderStress: The deviatoric part of the higher order stress.
        ...
        for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < dim; j++) {
                for (unsigned int k = 0; k < dim; k++) {
                    dDeviatoricHigherOrderStressdHigherOrderStress[tot_dim * dim * dim * i + tot_dim * dim * j +
                                                                   tot_dim * k + dim * dim * i + dim * j + k] += 1;
                    dDeviatoricHigherOrderStressdHigherOrderStress[tot_dim * dim * dim * i + tot_dim * dim * i +
                                                                   tot_dim * j + dim * dim * k + dim * k + j] -= 1. / 3;
                }
            }
        }
    ...
    void computeDeviatoricHigherOrderStress(const variableVector &higherOrderStress,
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricHigherOrderStress(
            higherOrderStress, deviatoricHigherOrderStress, _dDeviatoricHigherOrderStressdHigherOrderStress));

        dDeviatoricHigherOrderStressdHigherOrderStress =
            tardigradeVectorTools::inflate(_dDeviatoricHigherOrderStressdHigherOrderStress, tot_dim, tot_dim);
    ...
    void computeReferenceHigherOrderStressPressure(const variableVector &referenceHigherOrderStress,
        ...
        for (unsigned int AB = 0; AB < sot_dim; AB++) {
            for (unsigned int K = 0; K < dim; K++) {
                referenceHigherOrderPressure[K] +=
                    rightCauchyGreenDeformation[AB] * referenceHigherOrderStress[dim * AB + K];
            }
        }
    ...
    void computeReferenceHigherOrderStressPressure(const variableVector &referenceHigherOrderStress,
                                                   const variableVector &rightCauchyGreenDeformation,
                                                   variableVector &referenceHigherOrderPressure, variableMatrix &dpdM,
                                                   variableMatrix &dpdC) {
        /*!
         * Compute the pressure for a higher-order stress in the reference configuration.
         * \f$p_K = \frac{1}{3} C_{AB} M_{ABK}\f$
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeReferenceHigherOrderStressPressure(
            referenceHigherOrderStress, rightCauchyGreenDeformation, referenceHigherOrderPressure, _dpdM, _dpdC));
    ...
    void computeReferenceHigherOrderStressPressure(const variableVector &referenceHigherOrderStress,
                                                   const variableVector &rightCauchyGreenDeformation,
                                                   variableVector &referenceHigherOrderPressure, variableVector &dpdM,
                                                   variableVector &dpdC) {
        /*!
         * Compute the pressure for a higher-order stress in the reference configuration.
         * \f$p_K = \frac{1}{3} C_{AB} M_{ABK}\f$
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeReferenceHigherOrderStressPressure(referenceHigherOrderStress,
    ...
    void computeReferenceHigherOrderStressPressure(const variableVector &referenceHigherOrderStress,
                                                   const variableVector &rightCauchyGreenDeformation,
                                                   variableVector &referenceHigherOrderPressure, variableMatrix &dpdM,
                                                   variableMatrix &dpdC, variableMatrix &d2pdMdC) {
        /*!
         * Compute the pressure for a higher-order stress in the reference configuration.
         * \f$p_K = \frac{1}{3} C_{AB} M_{ABK}\f$
         *
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeReferenceHigherOrderStressPressure(referenceHigherOrderStress,
                                                                               rightCauchyGreenDeformation,
                                                                               referenceHigherOrderPressure, _dpdM,
                                                                               _dpdC, _d2pdMdC));
    ...
    void computeReferenceHigherOrderStressPressure(const variableVector &referenceHigherOrderStress,
                                                   const variableVector &rightCauchyGreenDeformation,
                                                   variableVector &referenceHigherOrderPressure, variableVector &dpdM,
                                                   variableVector &dpdC, variableVector &d2pdMdC) {
        /*!
         * Compute the pressure for a higher-order stress in the reference configuration.
         * \f$p_K = \frac{1}{3} C_{AB} M_{ABK}\f$
         *
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeReferenceHigherOrderStressPressure(
            referenceHigherOrderStress, rightCauchyGreenDeformation, referenceHigherOrderPressure, dpdM, dpdC));

        d2pdMdC = variableVector(dim * tot_dim * sot_dim, 0);
        for (unsigned int K = 0; K < dim; K++) {
            for (unsigned int N = 0; N < dim; N++) {
                for (unsigned int O = 0; O < dim; O++) {
                    d2pdMdC[tot_dim * sot_dim * K + dim * dim * dim * dim * N + dim * dim * dim * O + dim * dim * K +
                            dim * N + O] += 1. / 3;
                }
            }
        }
    ...
    void computeDeviatoricReferenceHigherOrderStress(const variableVector &referenceHigherOrderStress,
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeReferenceHigherOrderStressPressure(referenceHigherOrderStress,
                                                                               rightCauchyGreenDeformation, pressure));
    ...
    void computeDeviatoricReferenceHigherOrderStress(const variableVector &referenceHigherOrderStress,
        ...
        for (unsigned int I = 0; I < dim; I++) {
            for (unsigned int J = 0; J < dim; J++) {
                for (unsigned int K = 0; K < dim; K++) {
                    deviatoricReferenceHigherOrderStress[dim * dim * I + dim * J + K] -=
                        invRCG[dim * I + J] * pressure[K];
                }
            }
        }
    ...
    void computeDeviatoricReferenceHigherOrderStress(
        const variableVector &referenceHigherOrderStress, const variableVector &rightCauchyGreenDeformation,
        variableVector &deviatoricReferenceHigherOrderStress,
         ...
         * Compute the deviatoric part of the higher order stress in the reference configuration.
         *
         * \f$ \text{dev} ( M_{IJK} ) = M_{IJK} - \frac{1}{3} (C^{-1})_{IJ} C_{AB} M_{ABK} = M_{IJK} - ( C^{-1} )_{IJ}
         * p_{K} \f$
         *
         * Also compute Jacobians:
         * \f$ \frac{ \partial dev ( M_{IJK} ) }{ \partial M_{LMN} } = \delta_{IL} \delta_{JM} \delta_{KN} -
         * (C^{-1})_{IJ} \frac{ \partial p_K }{ \partial M_{LMN} } \f$
         *
         * \f$ \frac{ \partial dev ( M_{IJK} ) }{ \partial C_{LM} } = \left( (C^{-1})_{IL} (C^{-1})_{MJ} p_{K} -
         * (C^{-1})_{IJ} \frac{ \partial p_K }{ \partial C_{LM} } \right) \f$
         *
         * \param &referenceHigherOrderStress: The higher order stress in the reference configuration.
         * \param &rightCauchyGreenDeformation: The right Cauchy-Green deformation tensor.
         * \param &deviatoricReferenceHigherOrderStress: The deviatoric part of the higher order tensor in the
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricReferenceHigherOrderStress(
            referenceHigherOrderStress, rightCauchyGreenDeformation, deviatoricReferenceHigherOrderStress,
        ...
        dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress =
            tardigradeVectorTools::inflate(_dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress, 27, 27);
        dDeviatoricReferenceHigherOrderStressdRCG =
            tardigradeVectorTools::inflate(_dDeviatoricReferenceHigherOrderStressdRCG, 27, 9);
    ...
    void computeDeviatoricReferenceHigherOrderStress(
         ...
         *
         * Also compute Jacobians:
         * \f$ \frac{ \partial \text{ dev } ( M_{IJK} ) }{ \partial M_{LMN} } = \delta_{IL} \delta_{JM} \delta_{KN} -
         * (C^{-1})_{IJ} \frac{ \partial p_K }{ \partial M_{LMN} } \f$
         *
         * \f$ \frac{ \partial \text{ dev } ( M_{IJK} ) }{ \partial C_{LM} } = \left( (C^{-1})_{IL} (C^{-1})_{MJ} p_{K}
         * - (C^{-1})_{IJ} \frac{ \partial p_K }{ \partial C_{LM} } \right) \f$
         *
         * \param &referenceHigherOrderStress: The higher order stress in the reference configuration.
         * \param &rightCauchyGreenDeformation: The right Cauchy-Green deformation tensor.
        ...
        return computeDeviatoricReferenceHigherOrderStress(
            referenceHigherOrderStress, rightCauchyGreenDeformation, pressure, dpdM, dpdC,
            deviatoricReferenceHigherOrderStress, dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress,
            dDeviatoricReferenceHigherOrderStressdRCG);
    }

    void computeDeviatoricReferenceHigherOrderStress(
        const variableVector &referenceHigherOrderStress, const variableVector &rightCauchyGreenDeformation,
        const variableVector &pressure, const variableVector &dPressuredStress, const variableVector &dPressuredRCG,
        variableVector &deviatoricReferenceHigherOrderStress,
        variableVector &dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress,
        variableVector &dDeviatoricReferenceHigherOrderStressdRCG) {
        /*!
                ...
                for (unsigned int K = 0; K < dim; K++) {
                    for (unsigned int L = 0; L < dim; L++) {
                        for (unsigned int M = 0; M < dim; M++) {
                            for (unsigned int N = 0; N < dim; N++) {
                                dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress
                                    [dim * dim * tot_dim * I + dim * tot_dim * J + tot_dim * K + dim * dim * L +
                                     dim * M + N] -=
                                    invRCG[dim * I + J] * dPressuredStress[tot_dim * K + dim * dim * L + dim * M + N];
                            }

                            dDeviatoricReferenceHigherOrderStressdRCG[dim * dim * sot_dim * I + dim * sot_dim * J +
                                                                      sot_dim * K + dim * L + M] =
                                invRCG[dim * I + L] * invRCG[dim * M + J] * pressure[K] -
                                invRCG[dim * I + J] * dPressuredRCG[sot_dim * K + dim * L + M];
                        }
                    }
                }
            }
    ...
    void computeDeviatoricReferenceHigherOrderStress(
        ...
        variableVector &deviatoricReferenceHigherOrderStress,
        variableMatrix &dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress,
        variableMatrix &dDeviatoricReferenceHigherOrderStressdRCG, variableMatrix &d2DevMdMdRCG) {
        /*!
         * Compute the deviatoric part of the higher order stress in the reference configuration.
         *
         * \f$ \text{dev} ( M_{IJK} ) = M_{IJK} - \frac{1}{3} (C^{-1})_{IJ} C_{AB} M_{ABK} = M_{IJK} - ( C^{-1} )_{IJ}
         ...
         * (C^{-1})_{IJ} \frac{ \partial p_K }{ \partial C_{LM} } \right) \f$
         *
         * \f$ \frac{ \partial \text{dev} ( M_{IJK} ) }{ \partial M_{LMN} \partial C_{OP} } = (C^{-1})_{IO}
         * (C^{-1})_{PJ} \frac{ \partial p_{k }{ \partial M_{LMN} } - (C^{-1})_{IJ} \frac{ \partial^2 p_K}{ \partial
         * M_{LMN} \partial C_{OP} } \f$
         *
         * \param &referenceHigherOrderStress: The higher order stress in the reference configuration.
         * \param &rightCauchyGreenDeformation: The right Cauchy-Green deformation tensor.
         * \param &deviatoricReferenceHigherOrderStress: The deviatoric part of the higher order tensor in the
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricReferenceHigherOrderStress(
            referenceHigherOrderStress, rightCauchyGreenDeformation, deviatoricReferenceHigherOrderStress,
            _dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress,
            _dDeviatoricReferenceHigherOrderStressdRCG, _d2DevMdMdRCG));
    ...
    void computeDeviatoricReferenceHigherOrderStress(
        ...
        variableVector &deviatoricReferenceHigherOrderStress,
        variableVector &dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress,
        variableVector &dDeviatoricReferenceHigherOrderStressdRCG, variableVector &d2DevMdMdRCG) {
        /*!
         * Compute the deviatoric part of the higher order stress in the reference configuration.
         *
         * \f$ \text{dev} ( M_{IJK} ) = M_{IJK} - \frac{1}{3} (C^{-1})_{IJ} C_{AB} M_{ABK} = M_{IJK} - ( C^{-1} )_{IJ}
         ...
         * (C^{-1})_{IJ} \frac{ \partial p_K }{ \partial C_{LM} } \right) \f$
         *
         * \f$ \frac{ \partial dev( M_{IJK} ) }{ \partial M_{LMN} \partial C_{OP} } = (C^{-1})_{IO} (C^{-1})_{PJ} \frac{
         * \partial p_{k }{ \partial M_{LMN} } - (C^{-1})_{IJ} \frac{ \partial^2 p_K}{ \partial M_{LMN} \partial C_{OP}
         * } \f$
         *
         * \param &referenceHigherOrderStress: The higher order stress in the reference configuration.
         * \param &rightCauchyGreenDeformation: The right Cauchy-Green deformation tensor.
        ...
        return computeDeviatoricReferenceHigherOrderStress(
            referenceHigherOrderStress, rightCauchyGreenDeformation, pressure, dpdM, dpdC, d2pdMdRCG,
            deviatoricReferenceHigherOrderStress, dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress,
            dDeviatoricReferenceHigherOrderStressdRCG, d2DevMdMdRCG);
    }

    void computeDeviatoricReferenceHigherOrderStress(
        const variableVector &referenceHigherOrderStress, const variableVector &rightCauchyGreenDeformation,
        const variableVector &pressure, const variableVector &dPressuredStress, const variableVector &dPressuredRCG,
        const variableVector &d2PressuredStressdRCG, variableVector &deviatoricReferenceHigherOrderStress,
        variableVector &dDeviatoricReferenceHigherOrderStressdReferenceHigherOrderStress,
        variableVector &dDeviatoricReferenceHigherOrderStressdRCG, variableVector &d2DevMdMdRCG) {
        /*!
         * Compute the deviatoric part of the higher order stress in the reference configuration.
                            ...
                            for (unsigned int N = 0; N < dim; N++) {
                                for (unsigned int O = 0; O < dim; O++) {
                                    for (unsigned int P = 0; P < dim; P++) {
                                        d2DevMdMdRCG[dim * dim * tot_dim * sot_dim * I + dim * tot_dim * sot_dim * J +
                                                     tot_dim * sot_dim * K + dim * dim * dim * dim * L +
                                                     dim * dim * dim * M + dim * dim * N + dim * O + P] =
                                            invRCG[dim * I + O] * invRCG[dim * P + J] *
                                                dPressuredStress[tot_dim * K + dim * dim * L + dim * M + N] -
                                            invRCG[dim * I + J] *
                                                d2PressuredStressdRCG[tot_dim * sot_dim * K +
                                                                      dim * dim * dim * dim * L + dim * dim * dim * M +
                                                                      dim * dim * N + dim * O + P];
                                    }
                                }
                            }
    ...
    void computeDeviatoricSecondOrderStress(const variableVector &secondOrderStress,
        ...
        for (unsigned int i = 0; i < dim; i++) {
            deviatoricSecondOrderStress[dim * i + i] -= trace / 3.;
        }
    ...
    void computeDeviatoricSecondOrderStress(const variableVector &secondOrderStress,
         ...
         * \f$ \frac{\partial \hat{s}_{ij}}{\partial s_{kl} } \delta_{ik} \delta_{jl} - \frac{1}{3} \delta_{ij}
         * \delta_{kl} \f$
         *
         * \param &secondOrderStress: The stress measure in the current configuration.
         * \param &deviatoricSecondOrderStress: The deviatoric part of the second order stress
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricSecondOrderStress(secondOrderStress,
                                                                        deviatoricSecondOrderStress));
    ...
    void computeReferenceSecondOrderStressPressure(const variableVector &referenceStressMeasure,
        ...
        TARDIGRADE_ERROR_TOOLS_CHECK(
            referenceStressMeasure.size() == rightCauchyGreen.size(),
    ...
    void computeReferenceSecondOrderStressPressure(const variableVector &referenceStressMeasure,
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeReferenceSecondOrderStressPressure(referenceStressMeasure, rightCauchyGreen,
                                                                               pressure));
    ...
    void computeReferenceSecondOrderStressPressure(const variableVector &referenceStressMeasure,
        ...
        for (unsigned int i = 0; i < sot_dim; i++) {
            d2pdStressdRCG[sot_dim * i + i] = 1. / 3;
        }
    ...
    void computeDeviatoricReferenceSecondOrderStress(const variableVector &secondOrderReferenceStress,
         ...
         *
         * Also compute the Jacobians.
         * \f$\frac{ \partial \hat{S}_{IJ} }{ \partial S_{KL} } = \delta_{IK} \delta_{LJ} - \frac{1}{3} C_{KL}
         * (C^{-1})_{IJ}\f$
         *
         * \f$\frac{ \partial \hat{S}_{IJ} }{ \partial C_{KL} } = \frac{1}{3} \left( C_{AB} S_{AB} (C^{-1})_{IK}
         * (C^{-1})_{LJ} - S_{KL} (C^{-1}_{IJ}) \right)\f$
         *
         * \param &secondOrderReferenceStress: The stress measure in the reference configuration.
         * \param &rightCauchyGreenDeformation: The right Cauchy Green Deformation tensor of the
         *     deformation between configurations.
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricReferenceSecondOrderStress(
            secondOrderReferenceStress, rightCauchyGreenDeformation, deviatoricSecondOrderReferenceStress,
            _dDeviatoricReferenceStressdReferenceStress, _dDeviatoricReferenceStressdRCG));

        dDeviatoricReferenceStressdReferenceStress =
            tardigradeVectorTools::inflate(_dDeviatoricReferenceStressdReferenceStress, 9, 9);
    ...
    void computeDeviatoricReferenceSecondOrderStress(const variableVector &secondOrderReferenceStress,
        ...
        return computeDeviatoricReferenceSecondOrderStress(secondOrderReferenceStress, rightCauchyGreenDeformation,
                                                           pressure, dpdS, dpdC, deviatoricSecondOrderReferenceStress,
                                                           dDeviatoricReferenceStressdReferenceStress,
                                                           dDeviatoricReferenceStressdRCG);
    }

    void computeDeviatoricReferenceSecondOrderStress(
        const variableVector &secondOrderReferenceStress, const variableVector &rightCauchyGreenDeformation,
        const variableType &pressure, const variableVector &dPressuredStress, const variableVector &dPressuredRCG,
        variableVector &deviatoricSecondOrderReferenceStress,
        variableVector &dDeviatoricReferenceStressdReferenceStress, variableVector &dDeviatoricReferenceStressdRCG) {
        /*!
         * Compute the deviatoric part of a second order stress measure in the reference configuration.
         * \f$\hat{S}_{IJ} = S_{IJ} - \frac{1}{3} C_{AB} S_{AB} (C^{-1})_{IJ}\f$
         *
        ...
        for (unsigned int i = 0; i < sot_dim; i++) {
            dDeviatoricReferenceStressdReferenceStress[sot_dim * i + i] = 1.;
        }

        Eigen::Map<const Eigen::Matrix<variableType, 1, sot_dim, Eigen::RowMajor> > map2(dPressuredStress.data(), 1,
                                                                                         sot_dim);
        Eigen::Map<Eigen::Matrix<variableType, sot_dim, sot_dim, Eigen::RowMajor> > map3(
            dDeviatoricReferenceStressdReferenceStress.data(), sot_dim, sot_dim);
        Eigen::Map<const Eigen::Matrix<variableType, 1, sot_dim, Eigen::RowMajor> > map4(dPressuredRCG.data(), 1,
                                                                                         sot_dim);
        Eigen::Map<Eigen::Matrix<variableType, sot_dim, sot_dim, Eigen::RowMajor> > map5(
            dDeviatoricReferenceStressdRCG.data(), sot_dim, sot_dim);
        ...
        for (unsigned int I = 0; I < dim; I++) {
            for (unsigned int J = 0; J < dim; J++) {
                for (unsigned int K = 0; K < dim; K++) {
                    for (unsigned int L = 0; L < dim; L++) {
                        dDeviatoricReferenceStressdRCG[dim * sot_dim * I + sot_dim * J + dim * K + L] +=
                            pressure * invRCG[dim * I + K] * invRCG[dim * L + J];
                    }
                }
            }
        }
    ...
    void computeDeviatoricReferenceSecondOrderStress(
        const variableVector &secondOrderReferenceStress, const variableVector &rightCauchyGreenDeformation,
        const variableType &pressure, const variableVector &dPressuredStress, const variableVector &dPressuredRCG,
        const variableVector &d2PressuredStressdRCG, variableVector &deviatoricSecondOrderReferenceStress,
        variableVector &dDeviatoricReferenceStressdReferenceStress, variableVector &dDeviatoricReferenceStressdRCG,
         ...
         * (C^{-1})_{LJ} - S_{KL} (C^{-1}_{IJ}) \right)\f$
         *
         * \f$\frac{ \partial \hat{S}_{IJ} }{ \partial S_{KL} \partial C_{MN} } = - \frac{ \partial p^2 }{ \partial
         * S_{KL} \partial C_{MN}} (C^{-1})_{IJ} + \frac{ \partial p }{ \partial S_{KL} } (C^{-1})_{IM} (C^{-1})_{NJ}\f$
         *
         * \param &secondOrderReferenceStress: The stress measure in the reference configuration.
         * \param &rightCauchyGreenDeformation: The right Cauchy-Green deformation tensor of the
         *     deformation between configurations.
            ...
            for (unsigned int J = 0; J < dim; J++) {
                for (unsigned int K = 0; K < dim; K++) {
                    for (unsigned int L = 0; L < dim; L++) {
                        for (unsigned int M = 0; M < dim; M++) {
                            for (unsigned int N = 0; N < dim; N++) {
                                d2DevSdSdRCG[dim * sot_dim * sot_dim * I + sot_dim * sot_dim * J + dim * dim * dim * K +
                                             dim * dim * L + dim * M + N] =
                                    -d2PressuredStressdRCG[dim * sot_dim * K + sot_dim * L + dim * M + N] *
                                        invRCG[dim * I + J] +
    ...
    void computeDeviatoricReferenceSecondOrderStress(const variableVector &secondOrderReferenceStress,
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricReferenceSecondOrderStress(
            secondOrderReferenceStress, rightCauchyGreenDeformation, deviatoricSecondOrderReferenceStress,
            _dDeviatoricReferenceStressdReferenceStress, _dDeviatoricReferenceStressdRCG, _d2DevSdSdRCG));
    ...
    void computeDeviatoricReferenceSecondOrderStress(const variableVector &secondOrderReferenceStress,
        ...
        return computeDeviatoricReferenceSecondOrderStress(secondOrderReferenceStress, rightCauchyGreenDeformation,
                                                           ...
                                                           dDeviatoricReferenceStressdReferenceStress,
                                                           dDeviatoricReferenceStressdRCG, d2DevSdSdRCG);
    }
    ...
    void computeSecondOrderReferenceStressDecomposition(
        const variableVector &secondOrderReferenceStress, const variableVector &rightCauchyGreenDeformation,
        variableVector &deviatoricSecondOrderReferenceStress, variableType &pressure, variableMatrix &dDevStressdStress,
        variableMatrix &dDevStressdRCG, variableVector &dPressuredStress, variableVector &dPressuredRCG) {
        /*!
         * Compute the decomposition of a second-order stress measure into pressure and deviatoric parts.
         *
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeSecondOrderReferenceStressDecomposition(
            secondOrderReferenceStress, rightCauchyGreenDeformation, deviatoricSecondOrderReferenceStress, pressure,
            _dDevStressdStress, _dDevStressdRCG, dPressuredStress, dPressuredRCG));
    ...
    void computeSecondOrderReferenceStressDecomposition(
        const variableVector &secondOrderReferenceStress, const variableVector &rightCauchyGreenDeformation,
        variableVector &deviatoricSecondOrderReferenceStress, variableType &pressure, variableVector &dDevStressdStress,
        variableVector &dDevStressdRCG, variableVector &dPressuredStress, variableVector &dPressuredRCG) {
        /*!
         * Compute the decomposition of a second-order stress measure into pressure and deviatoric parts.
         *
         * Includes the first order Jacobians
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricReferenceSecondOrderStress(
            secondOrderReferenceStress, rightCauchyGreenDeformation, pressure, dPressuredStress, dPressuredRCG,
            deviatoricSecondOrderReferenceStress, dDevStressdStress, dDevStressdRCG));
    ...
    void computeSecondOrderReferenceStressDecomposition(
        ...
        variableVector &deviatoricSecondOrderReferenceStress, variableType &pressure, variableMatrix &dDevStressdStress,
        variableMatrix &dDevStressdRCG, variableVector &dPressuredStress, variableVector &dPressuredRCG,
        variableMatrix &d2DevStressdStressdRCG, variableMatrix &d2PressuredStressdRCG) {
        /*!
         * Compute the decomposition of a second-order stress measure into pressure and deviatoric parts.
         *
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeSecondOrderReferenceStressDecomposition(
            secondOrderReferenceStress, rightCauchyGreenDeformation, deviatoricSecondOrderReferenceStress, pressure,
            _dDevStressdStress, _dDevStressdRCG, dPressuredStress, dPressuredRCG, _d2DevStressdStressdRCG,
    ...
    void computeSecondOrderReferenceStressDecomposition(
        ...
        variableVector &deviatoricSecondOrderReferenceStress, variableType &pressure, variableVector &dDevStressdStress,
        variableVector &dDevStressdRCG, variableVector &dPressuredStress, variableVector &dPressuredRCG,
        variableVector &d2DevStressdStressdRCG, variableVector &d2PressuredStressdRCG) {
        /*!
         * Compute the decomposition of a second-order stress measure into pressure and deviatoric parts.
         *
         * Includes the first and some of the second order Jacobians
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricReferenceSecondOrderStress(
            secondOrderReferenceStress, rightCauchyGreenDeformation, pressure, dPressuredStress, dPressuredRCG,
            d2PressuredStressdRCG, deviatoricSecondOrderReferenceStress, dDevStressdStress, dDevStressdRCG,
    ...
    void computeHigherOrderReferenceStressDecomposition(const variableVector &higherOrderReferenceStress,
                                                        ...
                                                        variableMatrix &dDevStressdRCG,
                                                        variableMatrix &dPressuredStress,
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeHigherOrderReferenceStressDecomposition(
            higherOrderReferenceStress, rightCauchyGreenDeformation, deviatoricHigherOrderReferenceStress, pressure,
            _dDevStressdStress, _dDevStressdRCG, _dPressuredStress, _dPressuredRCG));
    ...
    void computeHigherOrderReferenceStressDecomposition(const variableVector &higherOrderReferenceStress,
                                                        ...
                                                        variableVector &dDevStressdRCG,
                                                        variableVector &dPressuredStress,
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricReferenceHigherOrderStress(
            higherOrderReferenceStress, rightCauchyGreenDeformation, pressure, dPressuredStress, dPressuredRCG,
            deviatoricHigherOrderReferenceStress, dDevStressdStress, dDevStressdRCG));

        return;
    }

    void computeHigherOrderReferenceStressDecomposition(
        const variableVector &higherOrderReferenceStress, const variableVector &rightCauchyGreenDeformation,
        variableVector &deviatoricHigherOrderReferenceStress, variableVector &pressure,
        variableMatrix &dDevStressdStress, variableMatrix &dDevStressdRCG, variableMatrix &dPressuredStress,
        variableMatrix &dPressuredRCG, variableMatrix &d2DevStressdStressdRCG, variableMatrix &d2PressuredStressdRCG) {
        /*!
         * Compute the decomposition of the higher-order stress measure into pressure and deviatoric parts.
         *
         * Also return the Jacobians
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeHigherOrderReferenceStressDecomposition(
            higherOrderReferenceStress, rightCauchyGreenDeformation, deviatoricHigherOrderReferenceStress, pressure,
            _dDevStressdStress, _dDevStressdRCG, _dPressuredStress, _dPressuredRCG, _d2DevStressdStressdRCG,
            _d2PressuredStressdRCG));
    ...
    void computeHigherOrderReferenceStressDecomposition(
        const variableVector &higherOrderReferenceStress, const variableVector &rightCauchyGreenDeformation,
        variableVector &deviatoricHigherOrderReferenceStress, variableVector &pressure,
        variableVector &dDevStressdStress, variableVector &dDevStressdRCG, variableVector &dPressuredStress,
        variableVector &dPressuredRCG, variableVector &d2DevStressdStressdRCG, variableVector &d2PressuredStressdRCG) {
        /*!
         * Compute the decomposition of the higher-order stress measure into pressure and deviatoric parts.
         *
         * Also return the Jacobians
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeDeviatoricReferenceHigherOrderStress(
            higherOrderReferenceStress, rightCauchyGreenDeformation, pressure, dPressuredStress, dPressuredRCG,
            d2PressuredStressdRCG, deviatoricHigherOrderReferenceStress, dDevStressdStress, dDevStressdRCG,
            d2DevStressdStressdRCG));
            ...
            for (unsigned int I = 0; I < 3; I++) {
                for (unsigned int J = 0; J < 3; J++) {
                    higherOrderStressNorm[K] +=
                        higherOrderStress[dim * dim * I + dim * J + K] * higherOrderStress[dim * dim * I + dim * J + K];
                }
            }
    ...
    void computeHigherOrderStressNorm(const variableVector &higherOrderStress, variableVector &higherOrderStressNorm,
                                      variableMatrix &dHigherOrderStressNormdHigherOrderStress, double tol) {
        /*!
         * Compute the norm of the higher order stress which is defined as
         * \f$|| M ||_K = \sqrt{ M_{IJK} M_{IJK} }\f$
         *
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeHigherOrderStressNorm(higherOrderStress, higherOrderStressNorm,
                                                                  _dHigherOrderStressNormdHigherOrderStress, tol));

        dHigherOrderStressNormdHigherOrderStress =
            tardigradeVectorTools::inflate(_dHigherOrderStressNormdHigherOrderStress, 3, 27);

        return;
    }
    void computeHigherOrderStressNorm(const variableVector &higherOrderStress, variableVector &higherOrderStressNorm,
                                      variableVector &dHigherOrderStressNormdHigherOrderStress, double tol) {
        /*!
         * Compute the norm of the higher order stress which is defined as
         * \f$|| M ||_K = \sqrt{ M_{IJK} M_{IJK} }\f$
         *
        ...
        for (unsigned int K = 0; K < 3; K++) {
            for (unsigned int L = 0; L < 3; L++) {
                for (unsigned int M = 0; M < 3; M++) {
                    dHigherOrderStressNormdHigherOrderStress[tot_dim * K + dim * dim * L + dim * M + K] =
                        higherOrderStress[dim * dim * L + dim * M + K] / (higherOrderStressNorm[K] + tol);
                }
            }
        }
    ...
    void computeHigherOrderStressNorm(const variableVector &higherOrderStress, variableVector &higherOrderStressNorm,
                                      ...
                                      variableMatrix &d2HigherOrderStressNormdHigherOrderStress2, double tol) {
        /*!
         * Compute the norm of the higher order stress which is defined as
         * \f$|| M ||_K = \sqrt{ M_{IJK} M_{IJK} }\f$
         *
         ...
         * \f$\frac{ \partial || M ||_K }{ \partial M_{LMN} } = \frac{ M_{LMK} \delta_{KN} }{ || M ||_K }\f$
         *
         * \f$\frac{ \partial^2 || M ||_K }{ \partial M_{LMN} \partial M_{OPQ} } = \frac{1}{ || M ||_K } \left[
         * \delta_{LO} \delta_{MP} \delta_{KQ} \delta_{KN} - \frac{ M_{LMK} \delta_{KN} }{ || M ||_K } \frac{ M_{OPK}
         * \delta_{KQ} }{ || M ||_K } \right]\f$
         *
         * where K is not summed over
         *
         * \param &higherOrderStress: The higher order stress tensor.
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeHigherOrderStressNorm(higherOrderStress, higherOrderStressNorm,
                                                                  ...
                                                                  _d2HigherOrderStressNormdHigherOrderStress2, tol));

        dHigherOrderStressNormdHigherOrderStress =
            tardigradeVectorTools::inflate(_dHigherOrderStressNormdHigherOrderStress, 3, 27);
        d2HigherOrderStressNormdHigherOrderStress2 =
            tardigradeVectorTools::inflate(_d2HigherOrderStressNormdHigherOrderStress2, 3, 27 * 27);
    ...
    void computeHigherOrderStressNorm(const variableVector &higherOrderStress, variableVector &higherOrderStressNorm,
                                      ...
                                      variableVector &d2HigherOrderStressNormdHigherOrderStress2, double tol) {
        /*!
         * Compute the norm of the higher order stress which is defined as
         * \f$|| M ||_K = \sqrt{ M_{IJK} M_{IJK} }\f$
         *
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(computeHigherOrderStressNorm(higherOrderStress, higherOrderStressNorm,
                                                                  dHigherOrderStressNormdHigherOrderStress, tol));
        ...
        for (unsigned int K = 0; K < dim; K++) {
            for (unsigned int L = 0; L < dim; L++) {
                for (unsigned int M = 0; M < dim; M++) {
                    d2HigherOrderStressNormdHigherOrderStress2[tot_dim * tot_dim * K + dim * dim * dim * dim * dim * L +
                                                               dim * dim * dim * dim * M + dim * dim * dim * K +
                                                               dim * dim * L + dim * M + K] +=
                        ...
                        for (unsigned int O = 0; O < dim; O++) {
                            for (unsigned int P = 0; P < dim; P++) {
                                for (unsigned int Q = 0; Q < dim; Q++) {
                                    d2HigherOrderStressNormdHigherOrderStress2[tot_dim * tot_dim * K +
                                                                               dim * dim * dim * dim * dim * L +
                                                                               dim * dim * dim * dim * M +
                                                                               dim * dim * dim * N + dim * dim * O +
                                                                               dim * P + Q] -=
                                        dHigherOrderStressNormdHigherOrderStress[tot_dim * K + dim * dim * L + dim * M +
                                                                                 N] *
                                        dHigherOrderStressNormdHigherOrderStress[tot_dim * K + dim * dim * O + dim * P +
                                                                                 Q] /
    ...
    void assembleGradientMicroDeformation(const variableVector &gradientMicroDisplacement,
        ...
        TARDIGRADE_ERROR_TOOLS_CATCH(assembleGradientMicroDeformation(gradientMicroDisplacement,
                                                                      gradientMicroDeformation));

        dGradChidGradPhi = variableVector(tot_dim * tot_dim, 0);
        for (unsigned int i = 0; i < tot_dim; i++) {
            dGradChidGradPhi[tot_dim * i + i] = 1;
        }

        return;
    }

    void dCauchyStressdPK2Stress(const variableVector &deformationGradient, variableVector &dCauchyStressdPK2Stress) {
        /*!
         * Compute the derivative of the Cauchy stress w.r.t. the PK2 stress
         *
         * \param &deformationGradient: The deformation gradient from the reference configuration to the current
         * configuration \param &dCauchyStressdPK2Stress: The derivative of the Cauchy stress w.r.t. the PK2 stress
         */
        ...
        for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < dim; j++) {
                for (unsigned int k = 0; k < dim; k++) {
                    for (unsigned int K = 0; K < dim; K++) {
                        dCauchyStressdPK2Stress[dim * sot_dim * i + sot_dim * j + dim * k + K] =
                            deformationGradient[dim * i + k] * deformationGradient[dim * j + K] / detF;
                    }
                }
            }
    ...
    void dCauchyStressdPK2Stress(const variableVector &deformationGradient, variableVector &dCauchyStressdPK2Stress,
        ...
        Eigen::Map<const Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> > map1(deformationGradient.data(), dim,
                                                                                       dim);
        Eigen::Map<Eigen::Matrix<variableType, dim, dim, Eigen::RowMajor> > map2(inverseDeformationGradient.data(), dim,
                                                                                 dim);
        detF = map1.determinant();
            ...
            for (unsigned int j = 0; j < dim; j++) {
                for (unsigned int k = 0; k < dim; k++) {
                    for (unsigned int K = 0; K < dim; K++) {
                        ...
                        dRdF[dim * sot_dim * sot_dim * i + sot_dim * sot_dim * j + dim * sot_dim * k + sot_dim * K +
                             dim * i + k] += deformationGradient[dim * j + K] / detF;

                        dRdF[dim * sot_dim * sot_dim * i + sot_dim * sot_dim * j + dim * sot_dim * k + sot_dim * K +
                             dim * j + K] += deformationGradient[dim * i + k] / detF;

                        for (unsigned int aA = 0; aA < sot_dim; ++aA) {
                            dRdF[dim * sot_dim * sot_dim * i + sot_dim * sot_dim * j + dim * sot_dim * k + sot_dim * K +
                                 aA] += -deformationGradient[dim * i + k] * deformationGradient[dim * j + K] *
    ...
    void dSymmetricMicroStressdReferenceSymmetricMicroStress(
         ...
         *
         * \param &deformationGradient: The deformation gradient from the reference configuration to the current
         * configuration \param &dSymmetricMicroStressdReferenceSymmetricMicroStress: The derivative of the symmetric
         * micro stress w.r.t. the reference symmetric micro stress
         */
    ...
    void dSymmetricMicroStressdReferenceSymmetricMicroStress(
        const variableVector &deformationGradient, variableVector &dSymmetricMicroStressdReferenceSymmetricMicroStress,
        variableVector &dRdF) {
        /*!
         * Compute the derivative of the symmetric micro stress w.r.t. the reference symmetric micro stress
         *
         * \param &deformationGradient: The deformation gradient from the reference configuration to the current
         * configuration \param &dSymmetricMicroStressdReferenceSymmetricMicroStress: The derivative of the symmetric
         * micro stress w.r.t. the reference symmetric micro stress \param &dRdF: The derivative of the result w.r.t.
    ...
    void dHigherOrderStressdReferenceHigherOrderStress(const variableVector &deformationGradient,
         ...
         *
         * \param &deformationGradient: The deformation gradient from the reference configuration to the current
         * configuration \param &microDeformation: The micro deformationt from the reference configuration to the
         * current configuration \param &dHigherOrderStressdReferenceHigherOrderStress: The derivative of the higher
         * order stress w.r.t. the reference higher order stress
         */
    ...
    void dHigherOrderStressdReferenceHigherOrderStress(const variableVector &deformationGradient,
                                                       const variableVector &microDeformation,
                                                       variableVector &dHigherOrderStressdReferenceHigherOrderStress,
                                                       variableVector &dRdF, variableVector &dRdChi) {
        /*!
         * Compute the derivative of the higher order stress w.r.t. the reference higher order stress
         *
         * \param &deformationGradient: The deformation gradient from the reference configuration to the current
         * configuration \param &microDeformation: The micro deformationt from the reference configuration to the
         * current configuration \param &dHigherOrderStressdReferenceHigherOrderStress: The derivative of the higher
         * order stress w.r.t. the reference higher order stress \param &dRdF: The derivative of the result w.r.t. the
         * deformation gradient \param &dRdChi: The derivative of the result w.r.t. the micro deformation
         */
                    ...
                    for (unsigned int l = 0; l < dim; l++) {
                        for (unsigned int M = 0; M < dim; M++) {
                            for (unsigned int N = 0; N < dim; N++) {
                                ...
                                dRdF[dim * dim * tot_dim * sot_dim * i + dim * tot_dim * sot_dim * j +
                                     tot_dim * sot_dim * k + dim * dim * sot_dim * l + dim * sot_dim * M + sot_dim * N +
                                     dim * i + l] +=
                                    deformationGradient[dim * j + M] * microDeformation[dim * k + N] / detF;

                                dRdF[dim * dim * tot_dim * sot_dim * i + dim * tot_dim * sot_dim * j +
                                     tot_dim * sot_dim * k + dim * dim * sot_dim * l + dim * sot_dim * M + sot_dim * N +
                                     dim * j + M] +=
                                    deformationGradient[dim * i + l] * microDeformation[dim * k + N] / detF;

                                dRdChi[dim * dim * tot_dim * sot_dim * i + dim * tot_dim * sot_dim * j +
                                       tot_dim * sot_dim * k + dim * dim * sot_dim * l + dim * sot_dim * M +
                                       sot_dim * N + dim * k + N] +=
                                    deformationGradient[dim * i + l] * deformationGradient[dim * j + M] / detF;
                                ...
                                for (unsigned int aA = 0; aA < sot_dim; ++aA) {
                                    dRdF[dim * dim * tot_dim * sot_dim * i + dim * tot_dim * sot_dim * j +
                                         ...
                                         sot_dim * N + aA] -=
                                        deformationGradient[dim * i + l] * deformationGradient[dim * j + M] *
                                        microDeformation[dim * k + N] * inverseDeformationGradient[aA] / detF;
                                }
                            }
                        }
                    }
